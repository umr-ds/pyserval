...
if __name__ == "__main__":
    ...
    identities = c.keyring.get_identities()
    print identities
    
    # # remove all but the first ServalIdentity
    # for identity in identities[1:]:
    #     identity.remove()

    new = c.keyring.add()
    print("    Added Id: {}".format(new.__repr__()))
    
    new.did = 1
    print("     Set did: {}".format(new.__repr__()))
    
    new.name = "PyServal Test"
    print("    Set name: {}".format(new.__repr__()))

    new.set(name="PyServal Test2", did=2)
    print("    Set both: {}".format(new.__repr__()))
